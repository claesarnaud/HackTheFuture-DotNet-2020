...
namespace TheFellowshipOfCode.DotNet.YourAdventure
{
    class NextItem
    {
        ...
        public NextItem(PlayTurnRequest _request, int _positionItemX, int _positionItemY)
        {
            request = _request;
        ...
        {
            if (request.PartyLocation.X > positionItemX && request.PossibleActions.Contains(TurnAction.WalkEast))
                return Task.FromResult(new Turn(TurnAction.WalkEast));
            if (request.PartyLocation.X < positionItemX && request.PossibleActions.Contains(TurnAction.WalkWest))
                return Task.FromResult(new Turn(TurnAction.WalkWest));
            if (request.PartyLocation.Y > positionItemY && request.PossibleActions.Contains(TurnAction.WalkSouth))
                return Task.FromResult(new Turn(TurnAction.WalkSouth));
            if (request.PartyLocation.Y < positionItemY && request.PossibleActions.Contains(TurnAction.WalkNorth))
